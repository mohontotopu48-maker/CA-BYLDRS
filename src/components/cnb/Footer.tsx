'use client';

import { Phone, Mail, MapPin, ChevronRight, Shield, Clock, Users } from 'lucide-react';
import { useRouter } from '@/lib/router-store';
import { footerLinks } from '@/lib/data';

export function Footer() {
  const { navigate } = useRouter();

  const handleNav = (href: string) => {
    navigate(href as import('@/lib/router-store').PageKey);
  };

  return (
    <footer className="bg-[#0d0906] text-[#9ba1a6]">
      {/* Main Footer */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          {/* Brand Column */}
          <div className="lg:col-span-1">
            <div className="relative inline-flex">
              <div className="absolute -inset-2 rounded-xl bg-gradient-to-r from-[#FF6B00] to-[#FF9F1C] opacity-30 blur-md" />
              <img
                src="/logo.png"
                alt="CA BYLDRS Logo"
                width={192} height={64}
                className="relative h-16 w-auto object-contain rounded-lg brightness-0 invert"
              />
            </div>
            <p className="text-sm text-[#9ba1a6] mb-6 leading-relaxed">
              Connecting Orange County &amp; Los Angeles County homeowners with verified, licensed local professionals. Built for trust, speed, and quality service matching.
            </p>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm text-[#9ba1a6]">
                <MapPin className="h-4 w-4 text-[#FF6B00] shrink-0" />
                <span>Serving all of Orange &amp; LA Counties</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-[#9ba1a6]">
                <Phone className="h-4 w-4 text-[#FF6B00] shrink-0" />
                <a href="tel:+15629440500" className="hover:text-[#FF6B00] transition-colors">+1 562-944-0500</a>
              </div>
              <div className="flex items-center gap-2 text-sm text-[#9ba1a6]">
                <MapPin className="h-4 w-4 text-[#FF6B00] shrink-0" />
                <span>3400 Twilight Dr. Fullerton, CA 92835</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-[#9ba1a6]">
                <Mail className="h-4 w-4 text-[#FF6B00] shrink-0" />
                <a href="mailto:hello@nxlbyldr.com" className="hover:text-[#FF6B00] transition-colors">hello@nxlbyldr.com</a>
              </div>
            </div>
          </div>

          {/* Services Column */}
          <div>
            <h3 className="text-[#f0f2f5] font-semibold text-sm uppercase tracking-wider mb-5">
              Our Services
            </h3>
            <ul className="space-y-2.5">
              {footerLinks.services.map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => handleNav(link.href)}
                    className="text-sm text-[#9ba1a6] hover:text-[#FF6B00] transition-colors flex items-center gap-1.5 group"
                  >
                    <ChevronRight className="h-3 w-3 opacity-0 -ml-5 group-hover:opacity-100 group-hover:ml-0 transition-all" />
                    {link.label}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Locations Column */}
          <div>
            <h3 className="text-[#f0f2f5] font-semibold text-sm uppercase tracking-wider mb-5">
              Service Areas
            </h3>
            <ul className="space-y-2.5">
              {footerLinks.locations.map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => handleNav(link.href)}
                    className="text-sm text-[#9ba1a6] hover:text-[#FF6B00] transition-colors flex items-center gap-1.5 group"
                  >
                    <ChevronRight className="h-3 w-3 opacity-0 -ml-5 group-hover:opacity-100 group-hover:ml-0 transition-all" />
                    {link.label}
                  </button>
                </li>
              ))}
            </ul>

            <h3 className="text-[#f0f2f5] font-semibold text-sm uppercase tracking-wider mt-8 mb-5">
              Company
            </h3>
            <ul className="space-y-2.5">
              {footerLinks.company.map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => handleNav(link.href)}
                    className="text-sm text-[#9ba1a6] hover:text-[#FF6B00] transition-colors flex items-center gap-1.5 group"
                  >
                    <ChevronRight className="h-3 w-3 opacity-0 -ml-5 group-hover:opacity-100 group-hover:ml-0 transition-all" />
                    {link.label}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Trust & CTA Column */}
          <div>
            <h3 className="text-[#f0f2f5] font-semibold text-sm uppercase tracking-wider mb-5">
              Why Homeowners Trust Us
            </h3>
            <div className="space-y-4 mb-8">
              <div className="flex items-start gap-3">
                <Shield className="h-5 w-5 text-[#FF6B00] shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm text-[#f0f2f5] font-medium">Licensed &amp; Verified</p>
                  <p className="text-xs text-[#9ba1a6]">Every pro is background-checked</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Clock className="h-5 w-5 text-[#FF6B00] shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm text-[#f0f2f5] font-medium">Fast Response</p>
                  <p className="text-xs text-[#9ba1a6]">Quick matching &amp; follow-up</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Users className="h-5 w-5 text-[#FF6B00] shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm text-[#f0f2f5] font-medium">Local Matching</p>
                  <p className="text-xs text-[#9ba1a6]">County-based service matching</p>
                </div>
              </div>
            </div>

            {/* Social links placeholder */}
            <div className="flex items-center gap-3 mb-6">
              <span className="text-xs text-[#9ba1a6] uppercase tracking-wider font-semibold">Follow Us</span>
              <div className="flex gap-2">
                <div className="w-8 h-8 rounded-lg bg-[#2a1f14] border border-white/[0.06] flex items-center justify-center text-[#9ba1a6] hover:text-[#FF6B00] hover:border-[#FF6B00]/30 transition-all cursor-pointer">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/></svg>
                </div>
                <div className="w-8 h-8 rounded-lg bg-[#2a1f14] border border-white/[0.06] flex items-center justify-center text-[#9ba1a6] hover:text-[#FF6B00] hover:border-[#FF6B00]/30 transition-all cursor-pointer">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z"/></svg>
                </div>
                <div className="w-8 h-8 rounded-lg bg-[#2a1f14] border border-white/[0.06] flex items-center justify-center text-[#9ba1a6] hover:text-[#FF6B00] hover:border-[#FF6B00]/30 transition-all cursor-pointer">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                </div>
              </div>
            </div>

            <button
              onClick={() => handleNav('partner')}
              className="w-full bg-gradient-to-r from-[#FF6B00] to-[#FF9F1C] hover:from-[#FF9F1C] hover:to-[#FF6B00] text-white text-sm font-semibold py-3 px-5 rounded-xl transition-all cta-glow"
            >
              Become a Partner
            </button>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-[#9ba1a6]">
            &copy; 2024 CA BYLDRS. All rights reserved.
          </p>
          <div className="flex items-center gap-6">
            <button
              onClick={() => handleNav('privacy')}
              className="text-xs text-[#9ba1a6] hover:text-[#f0f2f5] transition-colors"
            >
              Privacy Policy
            </button>
            <button
              onClick={() => handleNav('terms')}
              className="text-xs text-[#9ba1a6] hover:text-[#f0f2f5] transition-colors"
            >
              Terms of Service
            </button>
          </div>
        </div>
      </div>
    </footer>
  );
}
